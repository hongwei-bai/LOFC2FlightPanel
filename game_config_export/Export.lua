...
--bhw
-- 1) File
	local file = io.open("/log_lofc2_bhw.log", "w")
	if file then
		io.output(file)
	end
-- 2) Socket
	package.path  = package.path..";.\\LuaSocket\\?.lua"
	package.cpath = package.cpath..";.\\LuaSocket\\?.dll"
	io.write(string.format("package init complete"))
	socket = require("socket")
	host = host or "localhost"
	port = port or 8044
	c = socket.try(socket.connect(host, port)) -- connect to the listener socket
...
--bhw
	...
	local indAirSpeed = LoGetIndicatedAirSpeed()
	local playerPlaneId = LoGetPlayerPlaneId()
	local object = LoGetObjectById(playerPlaneId)
...
-- 2) Socket
	...
	--mission
	retTable["playerPlaneId"] = playerPlaneId
	retTable["country"] = object.Country
	retTable["level1"] = object.Type.level1
	retTable["level2"] = object.Type.level2
	retTable["level3"] = object.Type.level3
	retTable["level4"] = object.Type.level4

	--flight
	retTable["trueAirSpeed"] = trueAirSpeed
	retTable["indAirSpeed"] = indAirSpeed
	retTable["altBar"] = altBar
	retTable["engineLeft"] = engine.RPM.left
	retTable["engineRight"] = engine.RPM.right
	retTable["verticalSpeed"] = verticalSpeed

	--position
	retTable["gear"] = mechInfo.gear.status
	retTable["flaps"] = mechInfo.flaps.status
	retTable["speedbrakes"] = mechInfo.speedbrakes.status
	retTable["hook"] = mechInfo.hook.status
	
	--mechanization
	retTable["pitch"] = 57.3*pitch

	--将表数据编码成json字符串
	local jsonStr = cjson.encode(retTable);
	socket.try(c:send(jsonStr.."\n"))
end
...
--	  io.write(string.format("ID = %d, position = (%f,%f,%f) , V = (%f,%f,%f),flags = 0x%x\n",cur.target.ID,cur.target.position.p.x,cur.target.position.p.y,cur.target.position.p.z,cur.target.velocity.x,cur.target.velocity.y,cur.target.velocity.z,cur.target.flags))
--	end
